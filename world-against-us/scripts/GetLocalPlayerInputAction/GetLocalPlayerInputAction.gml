...
{
	// DISABLE ACTIONS IF NOT DEFAULT POSTURE DUE TO GLITCHING ANIMATIONS
	if (character.posture == character.default_posture)
	{
		...
		if (is_undefined(actionHandlerRef.active_action))
		{
			...
			{
				if (actionHandlerRef.GetActionIndex() != CHARACTER_ACTION.SHOOT)
				{
			...
			{
				if (actionHandlerRef.GetActionIndex() != CHARACTER_ACTION.SWITCH_FIRING_MODE)
				{
					if (!actionHandlerRef.IsActionInCooldown(CHARACTER_ACTION.SWITCH_FIRING_MODE))
					{
